...
「谛听」：本地运行的 Windows 小工具，轮询监控指定用户在**东方财富股吧 / 推特(X) / 微博**上的新动态，弹 Windows 系统通知并显示在列表里。界面是本机 HTTP 服务 + 浏览器页面（`server.py` + `web/`）。纯 Python 标准库 + 可选 `winotify`，前端 React + htm 全部本地 vendor、零构建，无第三方推送服务依赖。旧的 tkinter 窗口版和微信推送命令行版已于 2026-09-14 删除（不再需要多端推送），别再加回来。
...
```bash
python -c "import py_compile; [py_compile.compile(f, doraise=True) for f in ('server.py','core.py','monitor.py','mock_data.py')]"   # 语法检查
python -m pyflakes server.py core.py monitor.py mock_data.py    # 未用导入/未定义名字（pip install pyflakes）
...
验证 `core.py` 的行为（首轮基线 / 重启补发 / 去重 / 静音 / 停止 / 抓取健康度）不要靠真实联网。用 monkeypatch 跳过配置和联网：

```python
monitor.DB_PATH = <临时文件>; monitor.STATE_PATH = <临时文件>; monitor.LOG_PATH = <临时文件>   # 别碰真实 messages.db
monitor.load_config = lambda: {"users": [...], "poll_interval_seconds": 2}   # 跳过 config.json（要在 import core 之前）
core.random.uniform = lambda a, b: 0; monitor.random.uniform = lambda a, b: 0  # 跳过用户间等待
monitor.parse_posts = 假函数; monitor.parse_replies = 假函数   # 让真实的 collect_items 参与，健康度才走得到
core.toast = lambda t, m, l=None: toasts.append(t)
# 然后 c = MonitorCore(); q = c.subscribe(); c.start(); sleep; c.stop(); 断言 q 里的事件序列和 c._status_payload()["health"]
```

注意 `monitor.collect_items(cfg, uid, errors)` 自己捕获发帖/评论两边的异常并追加进 `errors`（不抛），`core._run_loop` 靠这个 list 判断失败——假函数直接替换 `collect_items` 的话要记得接受第三个参数，否则 core 会把 `TypeError` 当成抓取失败。

`config.json` 不在版本库里（含真实 UID/Cookie），所以开发环境通常**没有**这个文件，`monitor.load_config()` 会抛 `FileNotFoundError`（故意不 `sys.exit`，因为它在后台线程/HTTP 线程里被调），`core.start()` 会把它变成给用户看的错误文案。

## 架构

### 一个核心、一个入口

- **`core.py`** — `MonitorCore`：后台轮询线程、首轮基线/去重、追加监视、静音/合并/toast、抓取健康度、写 `messages.db`、事件广播、读写用户设置。**无 UI 依赖**。三个数据源全支持。
...
- **`monitor.py`** — 抓取/解析/存储层：三个来源的解析函数、`collect_items()`、`config.json` / `state.json` 读写、`messages.db` 存取、`log()`。无线程、无 UI，不知道 `MonitorCore` 的存在。

`core.py` 顶部的 `ENABLE_TWITTER` / `ENABLE_WEIBO` 目前是 `False`——推特/微博功能暂时下线（不轮询、UI 也不提），但代码和 `monitor.py` 里的抓取逻辑都完整保留，改成 `True` 即可恢复。改任何"用户列表"相关的地方（`core.py` 的 `_run_loop`/`list_users`/`save_users`/`manage_config`）时留意这两个开关，别让隐藏的来源重新泄漏到 UI，也别让保存逻辑遍历到没渲染出来的用户而误清空他们的配置。`list_users()` 和 `save_users()` 用同一份"启用来源"列表就是为了这个。

### 统一 item 字典是跨文件契约

`monitor.py` 的三个解析函数 `parse_posts()` / `parse_replies()`（股吧）、`parse_tweets()`（推特）、`parse_weibo()`（微博）都归一化成同一种 dict，`core._add_item()` 才能统一处理、前端才能混排渲染：
...
1. **`kind` 是中文字符串字面量**，且被当作 dict 键跨文件使用（`core._add_item()` / `_handle_new()` 的分支判断、`web/app.js` 的 `KINDS` 配色表）。取值：`发帖` / `转发` / `评论`（股吧、微博）、`推文` / `转推`（推特）、`追加`（`monitor.parse_post_appends()`，见下方「帖子追加检查」）。新增来源必须复用这些字符串，否则配色和通知文案会静默失配。
2. **`time` 必须是 `YYYY-MM-DD HH:MM:SS` 格式**。前端直接对字符串做切片（`time.slice(0,10)` 取日期分组、`slice(11,16)` 取显示时间），后端按字符串排序、翻页游标也按它比较，格式不对会导致分组、排序和翻页错乱。推特走 `_norm_time()`、微博走 `_parse_weibo_time()` 做归一化。
...
内存里的 `core.items` 上限 `MAX_ROWS`（1000），`snapshot(limit=300)` 首屏只给最近 300 条，更早的靠 `/api/items?before=<time>&before_key=<key>` 按 `(time, key)` 游标从库里翻——游标带 key 是为了同一秒多条时不重不漏。
...
`core.py` 单后台线程 `_run_loop()` 轮询，处理完的条目（已入 `core.items`、已写库、已弹通知）通过 `_broadcast()` 推给所有订阅者队列（`subscribe()` 拿，满 200 条丢最旧的，别让挂死的浏览器 tab 拖住后台线程）。事件形状：`("history"|"new", [entry...])`、`("status", {text, running, last_check, health})`、`("config", {users, config})`、`("cleared", {})`。
...
规划文档是 `docs/v2-design.md`。P2 里"删旧版 tkinter"已做（连同微信推送命令行版一起），"手机推送"**不做**（用户明确不需要多端推送）。已落地的几项跨文件契约：
...
- **类型**（`web/app.js` 的 `KINDS` / CSS 的 `--k-*` token）：卡片上的小 pill
- **用户色**（`config.json` 的 `color` 或 `groups`，8 个中国传统色见 `web/app.js` 的 `PALETTE`）：卡片左侧 4px 色条 + 用户名
