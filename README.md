...
- 「查追加」：勾选的用户发帖后 24 小时内，定期查帖子详情页有没有「作者更新」的追加内容，查到就当新动态提醒。详情页比列表接口更容易触发东财的验证页，所以这条路径带了失败退避（间隔按 2 倍拉长、封顶 2 小时，成功后复原）。
...
- 已有用户：点名字改备注，点色块换颜色，切换静音 / 查追加，删除（二次确认）。
- 底部改股吧轮询间隔和追加检查间隔。
...
- `color` 直接给用户指定颜色；`group` 引用 `groups` 里的颜色，同组同色。页面里的 8 个色块是几种中国传统色，手填别的十六进制也行。
...
**会不会被限流？** 列表接口不需要登录，默认 60 秒轮询、用户之间有随机间隔，正常用没遇到问题。帖子详情页（查追加用）更敏感，连续请求几次就可能被拦成验证页，程序会识别并退避，但别把追加检查间隔调得太短。轮询间隔下限 10 秒。
