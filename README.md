...
一个本地运行的小工具：盯住你关心的**东方财富股吧用户**和**推特(X)用户**，
他们一发帖 / 评论 / 发推，就弹 **Windows 系统通知** 并显示在页面列表里。
**不依赖任何第三方推送服务、没有每日条数上限。**

界面是本机跑的一个小网页（`127.0.0.1:17777`，不对外），用浏览器打开就行，放在竖屏副屏上正合适。
...
- **微博**：监控指定用户的发帖 / 转发 / 回复（需微博登录 Cookie）
- **查追加**：可选，帖子发布 24 小时内额外检查作者有没有**追加内容**，有追加就当新动态提醒，并顺延 24 小时继续查
- **Windows 系统通知** + 页面列表，点开卡片可看全文、一键打开原帖
- **收到的消息永久保存**（本地 SQLite），程序重启也不丢，打开页面就能看到历史，往上滚还能翻更早的
...
- 勾选 **「🔗查追加」**（仅股吧用户有这个选项）= 该用户**发帖后 24 小时内**额外检查作者有没有追加内容，查到追加就当新动态提醒、并把检查窗口再顺延 24 小时；超过窗口没有新追加就不再查这条帖子了（对应配置里的 `"check_appends": true`）。

保存后立即生效，设置会写回 `config.json`。

> 「查追加」要多发一次网络请求去读帖子详情页，所以默认关闭，只对你真正关心的人开。检查频率由 `append_check_interval_seconds` 控制（默认 300 秒），可以在 `config.json` 顶层手动加这一项调整。
...
配置里加这三项，抓取用户的发帖 / 转发 / 回复（紫色系，和股吧/推特混排）：

```json
  "weibo_cookie": "SUB=...; SUBP=...",
  "weibo_poll_interval_seconds": 120,
  "weibo_users": [
    { "uid": "1401527553", "name": "某某" }
  ]
```

- `uid`：微博主页 `weibo.com/u/后面那串数字`。
- `weibo_cookie`：**登录**微博后导出的 Cookie（至少 `SUB`，建议连 `SUBP` 一起）。用 Cookie-Editor 或 F12→Network 复制。
- 微博 `SUB` 通常能管**数周**，失效（改密/异地退登）后重新导一次即可。数据走桌面版 `weibo.com/ajax/statuses/mymblog` 接口。
...
- **点卡片**展开全文，里面有「打开原帖」；
- 顶栏「🔔 测试通知」可先验证系统通知是否正常；「设置」改配色 / 静音 / 查追加；
- **关掉浏览器标签页程序不会退出**（继续在后台监控），要停就点顶栏的「退出程序」，或再打开 `http://127.0.0.1:17777`；
- 全程本地运行，服务只监听本机回环地址，**不需要任何推送 key，没有每天条数上限**。
...
python server.py                       # 或双击 run_gui.bat
```
