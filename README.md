...
- **收到的消息永久保存**（本地 SQLite），程序重启也不丢，开窗口就能看到历史
...
>
> 想要 Windows 11 风格的现代圆角界面，再装一个可选依赖：`pip install sv_ttk`。
> 没装也能正常用，只是界面会是旧版扁平样式。

> **每次启动**：会先把之前保存过的消息（`messages.db`）直接加载进列表，同时后台开始检查有没有新动态；
...
- （可选）现代界面皮肤：`pip install sv_ttk`（不装也能用，只是界面回退到旧版扁平样式）
